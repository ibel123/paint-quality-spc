...
header-includes:
  ...
  - \onehalfspacing
  - \captionsetup{labelfont=it, textfont=it, position=below, font=small}
  - \captionsetup[table]{labelfont=it, textfont=it, position=below}
...
pkgs <- c(
  ...
  "gridExtra", "GGally", "car", "mvtnorm", "bookdown"
)
...
library(ggplot2); library(qcc); library(MASS)
library(FactoMineR); library(factoextra); library(knitr)
library(kableExtra); library(dplyr); library(tidyr)
library(ggExtra); library(gridExtra); library(GGally)
library(car); library(mvtnorm); library(bookdown)
```
...
% If you have the Makerere crest, replace the line below with:
% \includegraphics[width=1.5in]{makerere_logo.png}\\[0.15in]
{\Large \textbf{MAKERERE} \hspace{0.5cm} \textbf{UNIVERSITY}}\\[0.25in]

\rule{\linewidth}{0.4pt}\\[0.15in]

{\normalsize \textbf{COLLEGE OF BUSINESS AND MANAGEMENT SCIENCES (COBAMS)}}\\[0.08in]
{\normalsize \textbf{SCHOOL OF STATISTICS AND PLANNING}}\\[0.4in]

\rule{\linewidth}{0.4pt}\\[0.4in]

% ── Report title ───────────────────────────────────────────────
{\LARGE \textbf{MULTIVARIATE STATISTICAL PROCESS CONTROL}}\\[0.15in]
{\LARGE \textbf{OF PAINT QUALITY INDICATORS}}\\[0.2in]
{\large \textit{A Hotelling's T\textsuperscript{2} Approach}}\\[0.5in]

% ── Submission line ─────────────────────────────────────────────
{\normalsize By}\\[0.15in]
{\large \textbf{Group 3}}\\[0.6in]

% ── Details table ───────────────────────────────────────────────
\begin{tabular}{ll}
\textbf{Course:}     & Industrial Statistical Modeling \\[4pt]
\textbf{Lecturer:}   & Dr Saint Kizito Omala \\[4pt]
\textbf{Submitted:}  & \today \\[4pt]
\end{tabular}

\vfill

{\normalsize \textit{Submitted in Partial Fulfilment of the Requirements for the}}\\[0.06in]
{\normalsize \textit{Assessment of the Industrial Statistical Modeling Course Unit.}}
...
```{r load-data1}
d1 <- read.csv("Paint_data_individual.csv")
names(d1) <- c("Sample", "Viscosity", "Temperature")
```
...
cat("Temperature chart -- Points beyond control limits:",
    paste(temp_beyond, collapse = ", "), "\n")
...
```{r hotelling-d1, fig.cap="Hotelling's $T^2$ control chart for Viscosity and Temperature jointly. The red dashed line denotes the UCL based on the chi-squared approximation (Phase II). Points above the UCL are flagged as multivariate out-of-control."}
...
kable(ooc_d1[, c("Sample", "Viscosity", "Temperature", "T2", "MD_distance")],
...
```{r load-data2}
d2 <- read.csv("paint_data_3vars.csv", row.names = 1)
names(d2) <- c("Viscosity", "Gloss", "DryingTime")
d2$Sample <- 1:nrow(d2)
```
...
```{r pairwise-scatter, fig.cap="Pairwise scatter plot matrix for all three paint quality variables in Dataset 2. The lower triangle shows bivariate scatter with LOESS smoothing; diagonal panels show marginal density plots; the upper triangle shows Pearson correlation coefficients.", fig.height=6, fig.width=7.5}
...
ggpairs(
  ...
  lower = list(continuous = wrap("smooth", se = TRUE, linewidth = 0.6)),
...
```{r univar-summary-d2, echo=FALSE}
...
cat("Viscosity OOC points  :", ifelse(length(uv_vis2)  == 0, "None",
    paste(uv_vis2,  collapse = ", ")), "\n")
cat("Gloss OOC points      :", ifelse(length(uv_gloss) == 0, "None",
    paste(uv_gloss, collapse = ", ")), "\n")
cat("Drying Time OOC points:", ifelse(length(uv_dry)   == 0, "None",
    paste(uv_dry,   collapse = ", ")), "\n")
```
...
```{r hotelling-d2, fig.cap="Hotelling's $T^2$ control chart for the trivariate paint quality dataset. Red points exceed the chi-squared UCL and are flagged as multivariate out-of-control signals."}
T2_df2 <- data.frame(
  Sample = 1:n2,
  T2     = T2_d2,
...
kable(decomp_df[, c("Sample", "T2_Full",
                    "Contrib_Vis", "Pct_Vis",
...
```{r decomp-plot, fig.cap="Stacked bar chart showing the absolute $T^2$ contributions of each variable for every out-of-control observation in Dataset 2. The dashed horizontal line represents the UCL; bars are coloured by contributing variable.", fig.height=4.5}
